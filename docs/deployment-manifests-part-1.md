...
* `instance_groups` lists the sets of instances that will run the same job templates/packages. Instance groups will be deployed as either long running instances or errands (marked with `lifecycle: errand` and discussed later).
